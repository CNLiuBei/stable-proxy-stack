...
curl -fsSL "https://raw.githubusercontent.com/CNLiuBei/IFIM-Proxy/main/install.sh" -o /tmp/ifim-install.sh && \
bash /tmp/ifim-install.sh
```

或一行管道（脚本会自动落盘并从 `/dev/tty` 重跑）：
...
**忘了订阅页地址**，SSH 登录 VPS 后执行：

```bash
ifim-panel
```

（完整路径：`bash /etc/stable-proxy-stack/scripts/show-panel.sh`）
